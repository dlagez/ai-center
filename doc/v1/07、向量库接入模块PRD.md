...
## 21. Qdrant Local Mode 补充

为了降低本地开发和最小化 smoke 环境的启动成本，当前实现补充支持 Qdrant SDK 的本地模式。

### 21.1 目标

- 开发环境默认不要求单独启动 Qdrant Server
- 保持生产环境仍可切换到独立 Qdrant 服务或 Qdrant Cloud
- 不改变上层 `VectorStoreService`、`RetrieverService`、`KnowledgeIndexService`、`SimpleRAGService` 的调用方式
...
- 当 `QDRANT_LOCAL_MODE=true` 时，适配器走 `QdrantClient(path=...)`
- 当 `QDRANT_LOCAL_MODE=false` 时，适配器走远程 `QdrantClient(url=...)`
- `QDRANT_LOCAL_PATH` 支持相对项目根目录路径

### 21.3 使用建议

- 本地开发：推荐 `QDRANT_LOCAL_MODE=true`
- CI 单测：推荐 `QDRANT_LOCAL_MODE=true` 或临时目录模式
- 生产部署：推荐 `QDRANT_LOCAL_MODE=false`，接外部 Qdrant Server / Cloud

### 21.4 边界说明

Qdrant Local Mode 适合：
...
Qdrant Local Mode 不建议直接作为生产默认方案，原因包括：
